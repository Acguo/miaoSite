...
<div id="xlus"><div id="xlu">
<p id="box" style="display: none;">
<?php if($mjk!=""){ ?><button onClick="xldata(this)" data-jk="<?php echo $mjk; ?>">默认</button><?php } ?>
<?php
$jkjk=explode("\r\n",$mkcms_jiekou);
for($k=0;$k<count($jkjk);$k++){
$jkjk[$k]=explode('$',$jkjk[$k]);
echo '<button onclick="xldata(this)" data-jk="'.$jkjk[$k][1].'">'.$jkjk[$k][0].'</button>  ';
}
?>
</div></div>
...
<p class="desc margin-0"><span class="left text-muted"></span><?php echo $jian; ?></p>
</div>
</div>
</div>
</div>
...
<script>
 $(function () {
	               $.each($('.dianshijua'),function () {
                    ...
                    $('title').text(biaoti);
                    $('#xlus').children('button:eq(0)').addClass('jkbtn');
